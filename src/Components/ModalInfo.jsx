...
export default function ModalInfo({infoLabel, infoId, inputType}) {
    return (
        <div className={`flex flex-col sm:flex-row justify-start items-center p-3 pl-4 w-full`}>
            ...
            {
                inputType === 'textarea' ? 
                <textarea className={`sm:flex-grow pl-3 sm:p-2 text-lg border-[2px] border-blue-300 rounded resize-y overflow-scroll`} rows={`4`} cols={`30`} id={infoId} name={infoId}></textarea>:
                ``
            }
        </div>
    )
